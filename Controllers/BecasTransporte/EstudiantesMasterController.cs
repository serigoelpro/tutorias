...
{
    public class EstudiantesMasterController : Controller
    {
        private ModeloPlataforma db = new ModeloPlataforma();

        private string GetCarreraNombre(int idCarrera)
        {
            switch (idCarrera)
            {
                case 1: return "Tecnologías de la Información";
                case 2: return "Mantenimiento Industrial";
                case 3: return "Mecatrónica";
                case 4: return "Administración";
                case 5: return "Industrial";
                case 6: return "Energías Renovables";
                case 7: return "Logística";
                case 8: return "Logística Internacional";
                case 9: return "Aeronáutica en Manufactura";
                default: return "Carrera no definida";
            }
        }
        ...
        public ActionResult Index()
        {
            ...
            var query = db.Usuarios.Where(u => u.IdNivel == 1 && u.Estado == true);

            if (usuarioActual != null && usuarioActual.IdNivel == 3)
            {
                query = query.Where(u => u.IdCarrera == usuarioActual.IdCarrera);
            }

            var estudiantes = query.ToList();

            ViewBag.CarrerasNombres = estudiantes.ToDictionary(
                e => e.IdUsuario,
                e => GetCarreraNombre(e.IdCarrera)
            );
        ...
        public ActionResult Details(int? id)
        {
            ...
            Usuario usuario = db.Usuarios.Where(u => u.IdNivel == 1 && u.Estado == true).FirstOrDefault(u => u.IdUsuario == id);
            if (usuario == null)
            {
                return HttpNotFound();
            }

            ViewBag.CarreraNombre = GetCarreraNombre(usuario.IdCarrera);
        ...
        public ActionResult Create()
        {
            ...
            {
                new { Value = 1, Text = "Tecnologías de la Información" },
                new { Value = 2, Text = "Mantenimiento Industrial" },
                new { Value = 3, Text = "Mecatrónica" },
                new { Value = 4, Text = "Administración" },
                new { Value = 5, Text = "Industrial" },
                new { Value = 6, Text = "Energías Renovables" },
                new { Value = 7, Text = "Logística" },
                new { Value = 8, Text = "Logística Internacional" },
                new { Value = 9, Text = "Aeronáutica en Manufactura" }
            }, "Value", "Text");
        ...
        public ActionResult Create([Bind(Include = "IdUsuario,NombreCompleto,UserName,Password,CorreoElectronico,IdCarrera,IdNivel,Tiempo,Estado")] Usuario usuario)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "IdUsuario,NombreCompleto,UserName,Password,CorreoElectronico,IdCarrera,IdNivel,Tiempo,Estado")] Usuario usuario)
        {
            if (ModelState.IsValid)
            {
                usuario.IdNivel = 1;
        ...
        public ActionResult GruposPorTutor()
        {
            ...
            if (tutor != null)
            {
                EstudiantesMasterController.IdCarrera = tutor.IdCarrera;
        ...
        public ActionResult Grupo(int Id)
        {
            ...
            EstudiantesMasterController.IdGrupo = Id;
        ...
        [HttpPost]
        public ActionResult AsignarBeca([Bind(Include = "Id,Nombre,ApellidoM,ApellidoP,Matricula,Sexo,Contraseña,IdTransporte,IdCarrera,IdGrado,IdGrupo,IdTurno,Direccion,IdBeca,MontoBeca, DetallesBecaEstudiante,MesesBeca")] Estudiante estudiante)
        {
            ViewBag.Becas = db.Becas.OrderBy(x => x.NombreBeca).ToList();
        ...
        public ActionResult Estadisticas()
        {
            ...
            using (ModeloPlataforma data = new ModeloPlataforma())
            {
                ...
                // --- Calculo Turnos para esa carrera, periodo, año ---
                var Turnos = data.TutoriaGrupals
                    .Where(x => x.IdCarrera == Id && x.Año == tiempo.Year && x.IdPeriodo == pa)
                    .Select(x => x.IdTurno)
                    .Distinct();

                // --- Totales ---
                int EstudiantesPorCarrera = data.DatosPersonales
                    .Where(x => Turnos.Contains(x.IdTurno))
                    .Count(x => x.IdCarrera == Id && x.IdPeriodo == pa && x.Año == DateTime.Now.Year && x.IdTurno > 0);
                ...
                // --- Cálculo estudiantesSinApoyo ---
                int estudiantesSinApoyo = data.Estudiantes
                    .Where(x => x.IdCarrera == Id
                                && (x.IdBeca == 0 || x.IdBeca == null)
                                && (x.IdTransporte == 0 || x.IdTransporte == null))
                    .Count();
                ...
                ViewBag.cantidadAlumnosSinApoyo = estudiantesSinApoyo; // <-- NUEVO
                ViewBag.IdCarrea = Id;
        ...
        [CustomAuthorize(Nivel = 3)]
        public ActionResult EstadisticasCoordinador()
        {
            Usuario user = (Usuario)Session["Usuario"];
            ...
            using (ModeloPlataforma data = new ModeloPlataforma())
            {
                ...
                ViewBag.cantidadAlumnosBeca = estadisticasBeca.Count();
                ViewBag.cantidadAlumnosTransporte = estadisticasTransporte.Count();
                ViewBag.IdCarrera = Id;
                ViewBag.NombreCarrera = data.Carreras.Find(Id).Nombre;
        ...
        public ActionResult EstadisticasMaster()
        {
            Usuario user = (Usuario)Session["Usuario"];
            ...
            using (ModeloPlataforma data = new ModeloPlataforma())
            {
                ...
                // Get all careers
                List<Carrera> carreras = data.Carreras.ToList();
                ViewBag.Carreras = carreras.Select(c => new SelectListItem
                {
                ...
                // Get all specialties
                List<Especialidad> especialidades = data.Especialidads.ToList();
                ViewBag.Especialidades = especialidades.Select(e => new SelectListItem
                {
                    Value = e.Id.ToString(),
                    Text = e.Nombre,
                    Group = new SelectListGroup { Name = data.Carreras.Find(e.IdCarrera).Nombre }
                }).ToList();

                // Global statistics - USANDO LA FORMA CORRECTA
                var totalEstudiantes = data.Estudiantes.Count();
                var totalBecados = data.Estudiantes.Where(x => x.IdBeca > 0).Count();
                var totalTransporte = data.Estudiantes.Where(x => x.IdTransporte > 0).Count();
                var totalSinApoyo = data.Estudiantes.Where(x => (x.IdBeca == 0 || x.IdBeca == null) && (x.IdTransporte == 0 || x.IdTransporte == null)).Count();

                ViewBag.TotalEstudiantesBecados = totalBecados;
                ViewBag.TotalEstudiantesTransporte = totalTransporte;
                ViewBag.TotalEstudiantesSinApoyo = totalSinApoyo;

                // Calculate total amount across all careers
                decimal montoTotalGeneral = 0;
                foreach (var carrera in carreras)
                {
                    string montoCarrera = montoTotalPorCuatrimestre(carrera.IdCarrera);
                    // Parse the currency string to get numeric value
                    if (decimal.TryParse(montoCarrera.Replace("$", "").Replace(",", ""), out decimal monto))
                    {
                        montoTotalGeneral += monto;
                    }
                }

                ViewBag.MontoTotalGeneral = montoTotalGeneral.ToString("C", new CultureInfo("es-MX"));

                return View();
            }
        }

        // NUEVO MÉTODO: Obtener datos para las gráficas usando la forma correcta
        [HttpPost]
        public JsonResult GetDatosParaGraficas()
        {
            try
            {
                using (var data = new ModeloPlataforma())
                {
                    var carreras = data.Carreras.ToList();
                    var datosGraficas = new List<object>();

                    foreach (var carrera in carreras)
                    {
                        // USANDO LA FORMA CORRECTA para obtener datos
                        var estudiantesBecados = data.Estudiantes
                            .Where(x => x.IdCarrera == carrera.IdCarrera && x.IdBeca > 0)
                            .Count();

                        var estudiantesTransporte = data.Estudiantes
                            .Where(x => x.IdCarrera == carrera.IdCarrera && x.IdTransporte > 0)
                            .Count();

                        datosGraficas.Add(new
                        {
                            idCarrera = carrera.IdCarrera,
                            nombreCarrera = carrera.Nombre,
                            codigoCarrera = GetCodigoCarrera(carrera.IdCarrera),
                            totalBecados = estudiantesBecados,
                            totalTransporte = estudiantesTransporte
                        });
                    }

                    // Datos para gráfica de tendencias (últimos 6 meses)
                    var datosTendencias = new List<object>();
                    var fechaActual = DateTime.Now;

                    // Obtener totales generales para simular tendencias
                    var totalBecas = data.Estudiantes.Where(x => x.IdBeca > 0).Count();
                    var totalTransporte = data.Estudiantes.Where(x => x.IdTransporte > 0).Count();

                    for (int i = 5; i >= 0; i--)
                    {
                        var fecha = fechaActual.AddMonths(-i);
                        var mes = fecha.ToString("MMM", new CultureInfo("es-ES"));

                        // Simular tendencia creciente basada en los totales
                        var factorMes = (6 - i) / 6.0; // Factor de 0 a 1
                        var becasMes = (int)(totalBecas * factorMes * (0.8 + (0.4 * new Random().NextDouble())));
                        var transporteMes = (int)(totalTransporte * factorMes * (0.8 + (0.4 * new Random().NextDouble())));

                        datosTendencias.Add(new
                        {
                            mes = mes,
                            becas = becasMes,
                            transporte = transporteMes
                        });
                    }
        ...
        // Método auxiliar para obtener códigos de carrera
        private string GetCodigoCarrera(int idCarrera)
        {
            switch (idCarrera)
            {
                case 1: return "TI";
        ...
        // NUEVO MÉTODO: Obtener estadísticas detalladas de becas y transporte por grupo
        [HttpPost]
        public JsonResult GetEstadisticasDetalladasPorGrupo(int idCarrera, int idGrado, int idGrupo, int idTurno)
        {
            try
            {
                using (var data = new ModeloPlataforma())
                {
                    // PRIMERA FASE: Obtener datos básicos del grupo
                    var grupo = data.TutoriaGrupals
                        .FirstOrDefault(x => x.IdCarrera == idCarrera &&
                                           x.IdGrado == idGrado &&
                                           x.IdGrupo == idGrupo &&
                                           x.IdTurno == idTurno);
                    ...
                    if (grupo == null)
                    {
                        return Json(new { error = "Grupo no encontrado" });
                    }

                    // SEGUNDA FASE: Obtener estadísticas detalladas usando la lógica que me mostraste
                    var alumnosConBeca = data.Estudiantes
                        .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                   x.IdGrado == grupo.IdGrado &&
                                   x.IdGrupo == grupo.IdGrupo &&
                                   x.IdTurno == grupo.IdTurno &&
                                   x.IdBeca > 0)
                        .Select(x => x.Matricula)
                        .ToList();

                    var alumnosConTransporte = data.Estudiantes
                        .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                   x.IdGrado == grupo.IdGrado &&
                                   x.IdGrupo == grupo.IdGrupo &&
                                   x.IdTurno == grupo.IdTurno &&
                    ...
                    var totalEstudiantes = data.Estudiantes
                        .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                   x.IdGrado == grupo.IdGrado &&
                                   x.IdGrupo == grupo.IdGrupo &&
                                   x.IdTurno == grupo.IdTurno)
                    ...
                    var estudiantesHombres = data.Estudiantes
                        .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                   x.IdGrado == grupo.IdGrado &&
                                   x.IdGrupo == grupo.IdGrupo &&
                                   x.IdTurno == grupo.IdTurno &&
                                   x.Sexo.ToUpper().StartsWith("H"))
                        .Count();

                    var estudiantesMujeres = data.Estudiantes
                        .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                   x.IdGrado == grupo.IdGrado &&
                                   x.IdGrupo == grupo.IdGrupo &&
                                   x.IdTurno == grupo.IdTurno &&
                                   x.Sexo.ToUpper().StartsWith("M"))
                        .Count();

                    // Obtener estadísticas de becas por sexo
                    var becadosHombres = data.Estudiantes
                        .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                   x.IdGrado == grupo.IdGrado &&
                                   x.IdGrupo == grupo.IdGrupo &&
                                   x.IdTurno == grupo.IdTurno &&
                    ...
                    var becadasMujeres = data.Estudiantes
                        .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                   x.IdGrado == grupo.IdGrado &&
                                   x.IdGrupo == grupo.IdGrupo &&
                                   x.IdTurno == grupo.IdTurno &&
                    ...
                    // Obtener estadísticas de transporte por sexo
                    var transporteHombres = data.Estudiantes
                        .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                   x.IdGrado == grupo.IdGrado &&
                                   x.IdGrupo == grupo.IdGrupo &&
                                   x.IdTurno == grupo.IdTurno &&
                    ...
                    var transporteMujeres = data.Estudiantes
                        .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                   x.IdGrado == grupo.IdGrado &&
                                   x.IdGrupo == grupo.IdGrupo &&
                                   x.IdTurno == grupo.IdTurno &&
                    ...
                    // Calcular estudiantes sin apoyo
                    var estudiantesSinApoyo = totalEstudiantes - alumnosConBeca.Count - alumnosConTransporte.Count;

                    return Json(new
                    {
                        success = true,
                        grupo = $"{grupo.Grado.Nombre}{grupo.Grupo.Nombre}",
                        ...
                        totalBecados = alumnosConBeca.Count,
                        becadosHombres = becadosHombres,
                        becadasMujeres = becadasMujeres,
                        totalTransporte = alumnosConTransporte.Count,
                        transporteHombres = transporteHombres,
                        transporteMujeres = transporteMujeres,
                        estudiantesSinApoyo = estudiantesSinApoyo,
                        montoTotal = montoTotalPorCuatrimestrePorGrupo(
                            data.Estudiantes.Where(x => x.IdCarrera == grupo.IdCarrera &&
                                                       x.IdGrado == grupo.IdGrado &&
                                                       x.IdGrupo == grupo.IdGrupo &&
                                                       x.IdTurno == grupo.IdTurno)
                                           .Select(x => new DatosPersonales { Matricula = x.Matricula })
                                           .ToList(),
                            alumnosConBeca
                        )
                    });
        ...
        // NUEVO MÉTODO: Obtener estadísticas generales de todas las carreras
        [HttpPost]
        public JsonResult GetEstadisticasGeneralesTodasCarreras()
        {
            try
            {
                using (var data = new ModeloPlataforma())
                {
                    var estadisticasGenerales = new List<object>();
                    ...
                    foreach (var carrera in carreras)
                    {
                        // Obtener todos los grupos de la carrera
                        var grupos = data.TutoriaGrupals
                            .Where(x => x.IdCarrera == carrera.IdCarrera)
                            .ToList();

                        var totalEstudiantesCarrera = 0;
                        var totalBecadosCarrera = 0;
                        var totalTransporteCarrera = 0;
                        var montoTotalCarrera = 0m;

                        foreach (var grupo in grupos)
                        {
                            // Aplicar la lógica que me mostraste para cada grupo
                            var alumnosConBeca = data.Estudiantes
                                .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                           x.IdGrado == grupo.IdGrado &&
                                           x.IdGrupo == grupo.IdGrupo &&
                            ...
                            var totalEstudiantesGrupo = data.Estudiantes
                                .Where(x => x.IdCarrera == grupo.IdCarrera &&
                                           x.IdGrado == grupo.IdGrado &&
                                           x.IdGrupo == grupo.IdGrupo &&
                                           x.IdTurno == grupo.IdTurno)
                            ...
                            totalEstudiantesCarrera += totalEstudiantesGrupo;
                            totalBecadosCarrera += alumnosConBeca.Count;
                            totalTransporteCarrera += alumnosConTransporte.Count;
                        }
                        ...
                        if (decimal.TryParse(montoCarrera.Replace("$", "").Replace(",", ""), out decimal monto))
                        {
                            montoTotalCarrera = monto;
                        }

                        estadisticasGenerales.Add(new
                        {
                            idCarrera = carrera.IdCarrera,
                            nombreCarrera = carrera.Nombre,
                            totalEstudiantes = totalEstudiantesCarrera,
                            totalBecados = totalBecadosCarrera,
                            totalTransporte = totalTransporteCarrera,
                            montoTotal = montoTotalCarrera.ToString("C", new CultureInfo("es-MX")),
                            porcentajeBecados = totalEstudiantesCarrera > 0 ? Math.Round((double)totalBecadosCarrera / totalEstudiantesCarrera * 100, 2) : 0,
                            porcentajeTransporte = totalEstudiantesCarrera > 0 ? Math.Round((double)totalTransporteCarrera / totalEstudiantesCarrera * 100, 2) : 0
                        });
                    }

                    return Json(new { success = true, data = estadisticasGenerales });
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GetEstadisticasPorCarrera(int idCarrera)
        {
            using (ModeloPlataforma data = new ModeloPlataforma())
            {
                #region Periodo Actual
                ...
                List<Estudiante> estadisticasBeca = data.Estudiantes.Where(x => x.IdCarrera == idCarrera && x.IdBeca > 0).ToList();
                List<Estudiante> estadisticasTransporte = data.Estudiantes.Where(x => x.IdCarrera == idCarrera && x.IdTransporte > 0).ToList();

                var Turnos = data.TutoriaGrupals.Where(x => x.IdCarrera == idCarrera && x.Año == tiempo.Year && x.IdPeriodo == pa).Select(x => x.IdTurno).Distinct();
                int EstudiantesPorCarrera = data.DatosPersonales.Where(x => Turnos.Contains(x.IdTurno)).Count(x => x.IdCarrera == idCarrera && x.IdPeriodo == pa && x.Año == DateTime.Now.Year && x.IdTurno > 0);
                ...
                return Json(new
                {
                    nombreCarrera = data.Carreras.Find(idCarrera).Nombre,
                    totalEstudiantes = EstudiantesPorCarrera,
                    estudiantesBecados = estadisticasBeca.Count(),
                    estudiantesTransporte = estadisticasTransporte.Count(),
                    estudiantesSinApoyo = estudiantesSinApoyo,
                    montoTotal = montoTotalPorCuatrimestre(idCarrera)
                });
            }
        }
        ...
        public JsonResult GetEspecialidadesPorCarrera(int idCarrera)
        {
            try
            {
                using (var data = new ModeloPlataforma())
                {
                    var especialidades = data.Especialidads
                        .Where(e => e.IdCarrera == idCarrera)
                        .Select(e => new { Value = e.Id, Text = e.Nombre })
                        .OrderBy(e => e.Text)
                        .ToList();
        ...
        public ActionResult DetallesBecaMaster()
        {
        ...
        public ActionResult _Graficas(int id)
        {
            CultureInfo ci = new CultureInfo("es-MX");
            Usuario user = (Usuario)Session["Usuario"];
            EstudiantesMasterController.idGrupoGrafica = id;
        ...
        public ContentResult GraficoBeca()
        {
            using (ModeloPlataforma data = new ModeloPlataforma())
            {
                int id = EstudiantesMasterController.idGrupoGrafica;
                string datosGrafica;
                if (id > 0)
                {
                    var grupo = data.TutoriaGrupals.FirstOrDefault(x => x.IdTutoriaGrupal == id);
                    EstudiantesMasterController.idGrupoGrafica = 0;
        ...
        public ActionResult _GraficasGrado(int id)
        {
            ...
            Usuario user = (Usuario)Session["Usuario"];
            CultureInfo ci = new CultureInfo("es-MX");
            EstudiantesMasterController.idGradoGrafica = id;
        ...
        public ContentResult GraficoBecaPorGrado()
        {
            ...
            using (ModeloPlataforma data = new ModeloPlataforma())
            {
                int id = EstudiantesMasterController.idGradoGrafica;
                string datosGrafica;
                if (id > 0)
                {
                    EstudiantesMasterController.idGradoGrafica = 0;
        ...
        public ActionResult _GraficasEspecialidad(int id)
        {
            ...
            EstudiantesMasterController.EspecialidadGrafico = especialidad;
            ...
            {
                return PartialView("_GraficasEspecialidad");
            }
        ...
        public ContentResult GraficoBecaPorEspecialidad()
        {
            ...
            using (ModeloPlataforma data = new ModeloPlataforma())
            {
                string especialidad = EstudiantesMasterController.EspecialidadGrafico;
                string datosGrafica;
                if (!String.IsNullOrEmpty(especialidad))
                {
                    EstudiantesMasterController.EspecialidadGrafico = null;
        ...
        public ActionResult MapaTransporte()
        {
            ...
            foreach (Transporte ruta in rutas)
            {
                ...
                int cantidad = db.Estudiantes.Where(x => x.IdTransporte == ruta.Id && x.IdCarrera == IdCarrera && x.IdTurno == 2 && x.periodoActual == pa && x.Año == tiempo.Year).Count();
        ...
        [HttpPost]
        public JsonResult GetEstadisticasDetalladas()
        {
            try
            {
                using (var data = new ModeloPlataforma())
                {
                    // Base de alumnos (usuarios de nivel alumno)
                    var alumnos = data.Usuarios.Where(u => u.IdNivel == 1 && u.Estado == true);

                    // Totales
                    int totalEstudiantes = alumnos.Count();

                    // Sexo desde DatosPersonales (join por UserName = Matricula)
                    int totalHombres =
                        (from u in alumnos
                         join d in data.DatosPersonales on u.UserName equals d.Matricula
                         where d.Sexo != null && d.Sexo.Trim() == "Hombre"
                         select u.IdUsuario).Count();

                    int totalMujeres =
                        (from u in alumnos
                         join d in data.DatosPersonales on u.UserName equals d.Matricula
                         where d.Sexo != null && d.Sexo.Trim() == "Mujer"
                         select u.IdUsuario).Count();

                    // Vulnerabilidad según IdEleccionVunerabilidad
                    var ei = data.EntrevistaInicials;

                    int vulnerablesEconomicos = ei.Count(x => x.IdEleccionVunerabilidad == 1);
                    int vulnerablesAcademicos = ei.Count(x => x.IdEleccionVunerabilidad == 2);
                    int vulnerablesPersonales = ei.Count(x => x.IdEleccionVunerabilidad == 3);
                    int noVulnerables = ei.Count(x => x.IdEleccionVunerabilidad == 4);
                    ...
                    int alumnosTrabajando = ei.Count(x => x.IdTrabajo == 1);
                    int alumnosBecados = data.Estudiantes.Count(x => x.IdBeca > 0);
                    ...
                    var estadisticas = new
                    {
                        ...
                        totalMujeres,
                        totalVulnerables,
                        vulnerablesEconomicos,
                        vulnerablesPersonales,
                        vulnerablesAcademicos,
                        noVulnerables,
                        padresFamilia,
                        alumnosTrabajando,
                        alumnosBecados
